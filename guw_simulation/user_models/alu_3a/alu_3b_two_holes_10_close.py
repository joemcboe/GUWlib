...
# add defects ----------------------------------------------------------------------------------------------------------
r = PHASED_ARRAY_RADIUS * 5.5
phi = 203.0*np.pi/180
pos_x = PLATE_WIDTH/2 + r * np.cos(phi)
pos_y = PLATE_WIDTH/2 + r * np.sin(phi)
defects = [Hole(position_x=pos_x, position_y=pos_y, diameter=10e-3)]

r = PHASED_ARRAY_RADIUS * 4.8
phi = 203.0*np.pi/180
pos_x = PLATE_WIDTH/2 + r * np.cos(phi)
pos_y = PLATE_WIDTH/2 + r * np.sin(phi)
defects.append(Hole(position_x=pos_x, position_y=pos_y, diameter=10e-3))
